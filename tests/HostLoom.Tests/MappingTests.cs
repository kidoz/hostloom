...
public sealed class MappingTests
{
    ...
    public void One_generic_map_class_registers_many_pairs_through_a_factory()
    {
        ...
        services.AddHostLoomMapping(mapping =>
        {
            ...
            AddEntityMap<SportEntity, SportModel, SportTranslation>(
                mapping,
                entity => new SportModel(entity.Name),
                model => new SportEntity(model.Name)
            );
            AddEntityMap<TeamEntity, TeamModel, TeamTranslation>(
                mapping,
                entity => new TeamModel(entity.Name),
                model => new TeamEntity(model.Name)
            );
        });
        using var provider = services.BuildServiceProvider(
            new ServiceProviderOptions { ValidateOnBuild = true, ValidateScopes = true }
        ...
        var forward = provider.GetRequiredService<IMapper<SportEntity, SportModel>>();
        var reverse = provider.GetRequiredService<IMapper<TeamModel, TeamEntity>>();
        Assert.Equal("football", forward.Map(new SportEntity("football")).Name);
        Assert.Equal("reds", reverse.Map(new TeamModel("reds")).Name);

        // The third type argument really closed the map; it is the parameter that makes an
        // open-generic registration impossible.
        Assert.Equal(
            typeof(SportTranslation),
            (
                (GenericEntityMapper<SportEntity, SportModel, SportTranslation>)forward
            ).TranslationType
        );
    }
    ...
    public sealed record SportEntity(string Name);

    public sealed record SportModel(string Name);

    public sealed record SportTranslation(string Text);

    public sealed record TeamEntity(string Name);

    public sealed record TeamModel(string Name);

    public sealed record TeamTranslation(string Text);
