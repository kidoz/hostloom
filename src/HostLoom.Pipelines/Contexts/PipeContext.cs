...
public class PipeContext(CancellationToken cancellationToken = default) : IPipeContext
{
    ...
    public bool HasPayload(Type payloadType)
    {
        ...
        if (payloadType.IsInstanceOfType(this)) return true;
        lock (_payloadLock) return FindPayload(payloadType) is not null;
    }
    ...
    public TPayload GetOrAddPayload<TPayload>(Func<TPayload> payloadFactory) where TPayload : class
    {
        ArgumentNullException.ThrowIfNull(payloadFactory);
        if (this is TPayload contextPayload) return contextPayload;

        lock (_payloadLock)
        {
            if (FindPayload(typeof(TPayload)) is TPayload existing) return existing;
            var payload = payloadFactory() ?? throw new InvalidOperationException("A payload factory cannot return null.");
            (_payloads ??= [])[typeof(TPayload)] = payload;
            return payload;
        }
    }

    public TPayload AddOrUpdatePayload<TPayload>(Func<TPayload> addFactory, Func<TPayload, TPayload> updateFactory)
    ...
    {
        if (_payloads is null) return null;
        if (_payloads.TryGetValue(payloadType, out var exactPayload)) return exactPayload;
