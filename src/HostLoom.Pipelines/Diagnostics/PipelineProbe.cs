...
{
    public static ProbeResult Inspect<TContext>(IPipe<TContext> pipe, CancellationToken cancellationToken = default)
    ...
    private sealed class ProbeContext(ProbeNode node, CancellationToken cancellationToken) : IProbeContext
    {
    ...
    private sealed class ProbeNode(string name)
    {
        ...
        public ProbeResult ToResult() => new(Name, Properties, Children.Select(child => child.ToResult()).ToList());
    }
}
