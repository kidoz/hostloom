...
`AddUpdate<TMapper>()` registers an update map — a class implementing
`IUpdateMapper<TSource, TDestination>`, which writes into an existing destination instead of
creating one — with the same inference, explicit-triple, factory, and instance overloads. Each
call registers exactly one contract, so a class that both creates and updates one pair goes
through `Add` and `AddUpdate`; the two are distinct services and coexist.
...
inject a closed `IMapper<TSource, TDestination>` there instead. The dispatcher never resolves an
update map, so inject `IUpdateMapper<TSource, TDestination>` directly. Registration uses closed generic
