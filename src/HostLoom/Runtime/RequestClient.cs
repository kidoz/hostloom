...
internal sealed class RequestClient<TRequest, TResponse>(
    ...
    IOptions<HostLoomOptions> options) : IRequestClient<TRequest, TResponse>
    where TRequest : IRequest<TResponse>
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var responseFrame = await broker.RequestAsync(
            destination,
        ...
        {
            throw new InvalidDataException($"Response correlation id '{response.CorrelationId}' did not match request '{requestId}'.");
        }

        if (response.Kind is MessageKind.Fault)
        {
            throw new RemoteRequestException(response.Fault ?? new RemoteFault("Unknown", "The remote endpoint returned an empty fault."));
        }

        if (response.Kind is not MessageKind.Response || response.MessageType != MessageTypeName.For<TResponse>())
        {
            throw new InvalidDataException($"Expected response '{MessageTypeName.For<TResponse>()}', received '{response.MessageType}'.");
        }

        return serializer.Deserialize<TResponse>(response.Body)
            ?? throw new InvalidDataException($"Response body for '{response.MessageType}' was null.");
    }
}
