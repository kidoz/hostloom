...
internal sealed class RequestExecutor<TRequest, TResponse>(
    ...
    IEnumerable<IRequestBehavior<TRequest, TResponse>> behaviors) : IRequestExecutor
    where TRequest : class, IRequest<TResponse>
{
    public async ValueTask<object?> ExecuteAsync(object request, CancellationToken cancellationToken)
    {
        var typedRequest = (TRequest)request;
        RequestHandlerDelegate<TResponse> pipeline = token => handler.HandleAsync(typedRequest, token);
