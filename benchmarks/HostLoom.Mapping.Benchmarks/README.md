...
- **The nested destinations differ by one allocation.** AutoMapper materialises a `List<T>` for the
  ...
  AutoMapper's own choice of destination type, not a disadvantage imposed by the benchmark, but it is
...
Run every mapping and strategy case once to verify discovery, setup, execution, and cleanup:
