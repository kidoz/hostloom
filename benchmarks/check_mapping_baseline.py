#!/usr/bin/env python3
"""Create or enforce the HostLoom mapping BenchmarkDotNet baseline."""
...
REPORT_NAMES = (
    "HostLoom.Mapping.Benchmarks.MappingBenchmarks-report-full-compressed.json",
    "HostLoom.Mapping.Benchmarks.MappingCollectionBenchmarks-report-full-compressed.json",
    "HostLoom.Mapping.Benchmarks.MapManyStrategyBenchmarks-report-full-compressed.json",
    "HostLoom.Mapping.Benchmarks.MappingLifetimeBenchmarks-report-full-compressed.json",
)
ENVIRONMENT_FIELDS = (
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Fail when mapping mean time or allocation regresses past the baseline."
    )
    parser.add_argument(
        "--results",
        type=Path,
    ...
    parser.add_argument(
        "--baseline",
        type=Path,
        default=Path("benchmarks/baselines/mapping.json"),
...
def load_reports(results: Path) -> tuple[dict[str, str], str, dict[str, dict[str, float]]]:
    ...
    if any(environment != environments[0] for environment in environments[1:]):
        raise ValueError("mapping reports were produced by different environments")
    ...
    for report in reports:
        for benchmark in report["Benchmarks"]:
            ...
            # Parameterised rows append " [Count=…]" to the job label; the job is the same.
            jobs.add(benchmark["DisplayInfo"].split(": ", 1)[1].split(" [", 1)[0])
    ...
    if len(jobs) != 1:
        raise ValueError("mapping reports used different BenchmarkDotNet jobs")
...
def check_baseline(
    ...
    print(
        f"Mapping baseline passed: {len(benchmarks)} benchmarks, "
